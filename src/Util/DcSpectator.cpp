...
namespace DcSpectator
{
    ...
    Player* FindWatchableByName(Player* viewer, std::string const& name)
    {
        ...
        if (Player* exact = ObjectAccessor::FindPlayerByName(name, false))
    ...
    bool SeatFollow(Player* viewer, Player* target, std::string* whyNot)
    {
        ...
        if (target->GetGUID() == current)
            return Refuse(whyNot, "Already watching " + target->GetName() + ".");
        ...
        {
            std::lock_guard<std::mutex> lock(g_mutex);
            auto it = g_spectators.find(viewer->GetGUID());
            if (it == g_spectators.end())
                return Refuse(whyNot, "Camera ended mid-switch.");
            it->second.watched = target->GetGUID();
            it->second.retargetAccumMs = 0;
        }

        Announce(viewer, "Now watching " + target->GetName() + ".");
        return true;
    }
